...
>在JRockit R28之前的版本中，并没有启用TLA，大对象是直接分配在堆中的。命令行参数`–XXlargeObjectLimit`用来指定所谓 **"大对象"**所占空间的最小值，默认值是2KB。从R28版本起，JRockit使用了更为灵活的 **浪费限制（waste limit）**来替代参数`–XXlargeObjectLimit`，空闲限制指定了当在TLA中分配大对象时，可以被浪费掉空间的最大值。
>
>因此，从R28版本起，在TLA中分配内存时，如果TLA的空闲空间中放不下对象，而且浪费限制的值小于TLA中剩余空闲空间的大小，则直接在堆中为对象分配内存；否则，JRockit会根绝目标对象的大小来决定是 **"浪费"**掉这个TLA区域，尝试将在一个新的TLA中尉对象分配内存，还是直接在堆中为对象分配内存。
