...
>     * 访问类型`T`声明的变量会执行类的初始化A static field declared by T is used and the field is not a constant variable ([§4.12.4]).
>     * 如果`T`是一个[顶层类（top level class）][4]，并且在词法上有断言语句内嵌其中，则会执行类的初始化      T is a top level class ([§7.6][4]), and an assert statement ([§14.10][5]) lexically nested within  T ([§8.1.3][6]) is executed.
>     * 通过`java.lang.reflect`包中类和`java.lang.Class`类以反射的方式调用Invocation of certain reflective methods in class `Class` and in package `java.lang.reflect` also causes class or interface initialization.
